...
import { createBillAction } from '@/app/bill-actions';
...
export function NewBillForm({
  ...
  const [state, formAction, pending] = useActionState<ActionResult, FormData>(createBillAction, {});
    ...
    <form action={formAction} className="space-y-5">
      <div className="rounded-lg border border-slate-200 bg-white p-5 flex flex-wrap gap-4">
        <label className="block">
          ...
          <select name="vendorId" required className="rounded-md border border-slate-300 px-2.5 py-1.5 text-sm bg-white">
        ...
        <label className="block">
          <span className="block text-xs font-medium text-slate-500 mb-1">Bill date</span>
          <input type="date" name="billDate" defaultValue={today} className="rounded-md border border-slate-300 px-2.5 py-1.5 text-sm" />
        </label>
        <label className="block">
          <span className="block text-xs font-medium text-slate-500 mb-1">Due date</span>
          <input type="date" name="dueDate" defaultValue={in30} className="rounded-md border border-slate-300 px-2.5 py-1.5 text-sm" />
        </label>
        <label className="block flex-1 min-w-56">
          <span className="block text-xs font-medium text-slate-500 mb-1">Memo</span>
          <input name="memo" placeholder="e.g. September rent" className="w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-sm" />
        </label>
        <label className="block">
      ...
      <button type="submit" disabled={pending} className="rounded-md bg-teal-600 px-5 py-2 text-sm font-medium text-white hover:bg-teal-700 disabled:opacity-50">
        {pending ? 'Saving…' : 'Save draft bill'}
      </button>
    </form>
  );
}
